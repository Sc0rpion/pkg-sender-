...
"function roleW(r){var u=(r||'').toUpperCase();return u==='GAME'?1:(u==='PATCH'?2:(u==='DLC'?3:4));}"
"function sortGames(a,b){var pa=a.platform==='PS5'?0:(a.platform==='PS4'?1:2);var pb=b.platform==='PS5'?0:(b.platform==='PS4'?1:2);if(pa!==pb)return pa-pb;"
"var fa=a.familyKey||a.titleId||'',fb=b.familyKey||b.titleId||'';if(fa&&fb&&fa===fb)return roleW(a.role)-roleW(b.role);"
"var ta=(a.title||'').toLowerCase(),tb=(b.title||'').toLowerCase();if(ta!==tb)return ta<tb?-1:1;return roleW(a.role)-roleW(b.role);}"
